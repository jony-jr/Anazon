...
import img from "@images/notFoundCart.png";
...
import DeleteFromWishBtn from "@/app/wishlist/_wishListBtn/DeleteFromWishBtn";
import DelHeartWish from "@/app/wishlist/_wishListBtn/DelHeartWish";

export default function ProductCard({ product, wishesIds }: { product: productType, wishesIds: string[] }) {
  console.log("🚀 ~ ProductCard ~ wishesIds:", wishesIds)
  const [isRedheart, setisRedheart] = useState(false)
  
  useEffect(() => {
    if (wishesIds.includes(product._id)) {
    setisRedheart(true)
  }else{
    setisRedheart(false)
  }
  ...
  return (
    <>
      <motion.div whileHover={{ scale: 1.02, transitionDuration: 0.5 }} className=" w-full max-w-sm relative bg-white border border-gray-200 rounded-2xl shadow-sm dark:bg-gray-800 dark:border-gray-700">
        ...
        <div className="px-5 py-5">
          ...
          <div className="flex items-center justify-between mt-2.5 mb-5">
            ...
            {isRedheart ?
              <DelHeartWish productId={product._id} />
              :
