...
import { addProductToWishlist } from "../wishlist.action";
import { FaRegHeart } from "react-icons/fa";
import { WishListContext } from "@/app/_Components/Contexts/WishListContext";

export default function AddToWishBtn({ className, productId }: { className?: string; productId: string }) {
    const { wishListIdes, updateWishesIdes } = useContext(WishListContext);
    const [isLoading, setIsLoading] = useState(false);

    async function handleAddToWishlist() {
        setIsLoading(true);
        // Optimistic update
        updateWishesIdes(productId);
        ...
        if (!isAdded) {
            ...
            wishListIdes.filter(id => id !== productId);
        } 
        setIsLoading(false);
    }

    return (
        <motion.div className="bg-transparent" whileTap={{ scale: 0.95 }} whileHover={{ scale: 1.1 }}>
            <Button
                style={{ padding: 0 }}
                ...
                disabled={isLoading}
            >
                <FaRegHeart size={30} className="text-black p-0 cursor-pointer hover:text-amber-600" />
            </Button>
        </motion.div>
    );
}